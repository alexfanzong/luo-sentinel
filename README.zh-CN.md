...
它的入口不是“人在搜索法律问题”，而是上游项目方 Agent 准备推进一个 RWA 动作。Sentinel 会先把这个动作扣住，限制在已审核的来源范围内，再逼到人工钱包关口，最后才生成有边界的下游 Agent 交接。

本项目的重点不是让 AI 直接给出“某地可以发行/转让”的法律结论，而是展示一条更安全的链路：先截停动作，确认资料来源和适用边界，再决定另一个 AI Agent 被允许做什么。
...
- **Agent 动作先截停**
  演示从一个拟执行的 Agent 动作开始，并在任何发行、转让、订单或策略真正运行前把它拦住。
