...
> 让项目方 Agent 在推进 RWA 动作前，先完成证据对齐、人工确认和可验证交接。
...
LUO Sentinel 是一个面向 Injective RWA 动作的证据对齐与交接层。它把经过审核的监管来源锚点组织成可视化证据地图，让 Agent 在进入链上执行前经过 Sentinel 评审、人工确认和可验证回执。

入口是上游项目方 Agent，不是人在搜索法律问题。当它准备推进一个 RWA 动作时，Sentinel 会把请求带回已审核的来源范围，保留执行前的人工确认，并在通过后生成有边界的下游 Agent 交接。

LUO Sentinel 不让 AI 直接给出“某地可以发行/转让”的法律结论。它展示的是一种更可靠的协作方式：Agent 把准备工作对齐到已审核来源，Sentinel 保留执行前的责任边界，由人确认下一步，再决定另一个 AI Agent 被允许做什么。
...
- **Agent 准备，Sentinel 对齐证据，人来拍板**
  演示从一个拟执行的 Agent 动作开始；系统先完成证据审核和人签字，再继续后续交接。
