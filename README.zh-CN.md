...
> 夹在项目方 Agent 和链上 RWA 执行之间的证据约束型 Sentinel 层。
...
LUO Sentinel 是一个面向 Injective RWA 动作的证据约束型 Sentinel 层。它把经过审核的监管来源锚点组织成可视化证据地图，并在任何下游 Agent 行动前加入 Sentinel 评审、人工关口和可验证回执。

来问的是上游项目方 Agent，不是人在搜索法律问题。当它准备推进一个 RWA 动作时，Sentinel 先让执行保持暂停，把请求对齐到已审核的来源范围，引入人工钱包关口，最后才生成有边界的下游 Agent 交接。

LUO Sentinel 不让 AI 直接给出“某地可以发行/转让”的法律结论。它展示的是一种更安全的分工：Agent 把准备对齐到已审核来源，Sentinel 把执行边界留在台面上，由人守住最后一关，再决定另一个 AI Agent 被允许做什么。
...
2. Sentinel 让执行保持暂停，并把动作带入证据约束的评审层。
3. 系统只允许它进入已审核的证据范围。
...
- **Agent 准备，Sentinel 守住边界，人来拍板**
  演示从一个拟执行的 Agent 动作开始；发行、转让、订单或策略都保持暂停，等证据审核和人签字后才继续。
